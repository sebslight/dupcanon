...
- **PRs** use both `comments` and `review_comment_count`
...
- Upsert repo in `repos`
- Upsert item rows in `items`
- Recompute `content_hash` on every upsert
...
- latest candidate set per source item (fresh by default)
...
| Activity signal in canonical tie-break | comment_count | comment_count + review_comment_count |
...
- both run through same deterministic gates
...
## Production-readiness gate for close automation

Before broad production apply, policy target is:

- **>= 0.90 precision**
- on **at least 100 labeled proposed closes**

This is separate from cheap-vs-strong model agreement and should be based on human-labeled close proposals.
