...
- Intent representation is now the default across batch + online flows (`embed|candidates|judge|judge-audit|canonicalize|plan-close|detect-new`). Use `--source raw` for rollback or A/B comparisons.
...
# optional recovery mode for transitive-only clusters:
# uv run dupcanon plan-close --repo openclaw/openclaw --type issue --target-policy direct-fallback --dry-run
...
# optional raw fallback path
# uv run dupcanon candidates --repo openclaw/openclaw --type issue --source raw
# uv run dupcanon judge --repo openclaw/openclaw --type issue --source raw
